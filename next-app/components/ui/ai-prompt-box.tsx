...
// Inject scrollbar styles once (client-side only)
...
const VoiceRecorder: React.FC<VoiceRecorderProps> = ({
  ...
  // timeRef mirrors time so we can read it in the stop callback without adding
  // `time` to the effect deps (which would re-fire onStopRecording every second)
  ...
  React.useEffect(() => {
    if (isRecording) {
      onStartRecording();
      timeRef.current = 0;
      // Reset the displayed counter without triggering the
      // "setState synchronously in effect body" lint rule — defer it.
  ...
  }, [isRecording, onStartRecording, onStopRecording]); // `time` intentionally excluded
  ...
  // Pre-compute the randomized bar heights / animation durations once on
  // mount via a lazy state initializer. This keeps the render phase pure —
  // otherwise `Math.random()` would re-run on every re-render (including the
  // per-second timer tick) and the bars would jitter unpredictably.
  const [bars] = React.useState<{ height: number; duration: number }[]>(() =>
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  ...
  React.useEffect(() => {
    if (document.getElementById("__jarvis-styles")) return;
    const el = document.createElement("style");
    el.id = "__jarvis-styles";
    el.textContent = SCROLLBAR_STYLES;
    document.head.appendChild(el);
  }, []);

  // Send is handled inside onresult so the transcript is always ready
  const handleStartRecording = React.useCallback(() => {
    ...
    rec.onresult = (ev: any) => {
      ...
      if (t) {
        ...
        onSend(messagePrefix ? `${messagePrefix}${t}]` : t, []);
      }
  ...
  }, [showSearch, showThink, showCanvas, onSend]);

  // Just stop the recogniser; transcript delivery happens via onresult
